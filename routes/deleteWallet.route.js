...
router.post("/", async (req, res) => {
  ...
  if (deleteWalletInfo.shiftWalletID == null)
    deleteWalletInfo.shiftWalletID = 0;
  console.log("Shift Wallet: " + deleteWalletInfo.shiftWalletID);

  if (
    deleteWalletInfo.walletID <= currentUser.wallets &&
    deleteWalletInfo.shiftWalletID <= currentUser.wallets
  ) {
    try {
      deleteWalletInfo.amount = currentUser.amounts[deleteWalletInfo.walletID];
      const deleteWalletFromFinancialTableQuery = `DELETE FROM "FINANCEMANAGER"."FinancialInfo"
	                                                 WHERE "UserID" LIKE '${currentUser.userID}'
	                                                 AND "WalletID" = ${deleteWalletInfo.walletID}`;
      let deleteWalletFromFinancialTableQueryResult = await runQuery(
        deleteWalletFromFinancialTableQuery
      );
      ...
      const result1 = await runProcedure(
        deleteWalletFromTablesQuery,
        deleteWalletFromTablesBinds
      );
      ...
      const result2 = await runProcedure(
        updateTablesAfterDeleteWalletQuery,
        updateTablesAfterDeleteWalletBinds
      );
      ...
      let shiftAmountToWalletQueryResult = await runQuery(
        shiftAmountToWalletQuery
      );
      currentUser.amounts[deleteWalletInfo.shiftWalletID] +=
        deleteWalletInfo.amount;
    } catch (error) {
      console.log(error);
      console.error(
        "deleteWalletFromTablesQuery/updateTablesAfterDeleteWalletQuery Not Executed"
      );
    }
  }
