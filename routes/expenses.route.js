...
const { getExpenseInfoPage, saveExpenseInfo } = require('../controllers/expenses.controller');
...
router.post("/", async(req, res)=>{
    ...
    if(newExpenseInfo.wallet <= currentUser.wallets){
        
        const expenseDateObject = new Date();
        let year = expenseDateObject.getFullYear(); let month = expenseDateObject.getMonth(); let dt = expenseDateObject.getDate();
        let hr = expenseDateObject.getHours(); let mm = expenseDateObject.getMinutes(); let ss = expenseDateObject.getSeconds();
        let expenseDateInput = String(year+'-'+month+'-'+dt+' '+hr+'-'+mm+'-'+ss);
