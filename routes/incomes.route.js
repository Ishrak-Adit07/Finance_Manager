...
const { getIncomeInfoPage, saveIncomeInfo } = require('../controllers/incomes.controller');
...
router.post("/", async(req, res)=>{
    ...
    if(newIncomeInfo.wallet <= currentUser.wallets){

        const incomeDateObject = new Date();
        let year = incomeDateObject.getFullYear(); let month = incomeDateObject.getMonth(); let dt = incomeDateObject.getDate();
        let hr = incomeDateObject.getHours(); let mm = incomeDateObject.getMinutes(); let ss = incomeDateObject.getSeconds();
        let incomeDateInput = String(year+'-'+month+'-'+dt+' '+hr+'-'+mm+'-'+ss);
        console.log(incomeDateInput);
        //New Income id = inc+userid+year+mon+date+hr+min+ss
        let newIncomeID = "inc_"+currentUser.userID+'_'+year+'.'+month+'.'+dt+'_'+hr+'.'+mm+'.'+ss;
        ...
        const newIncomeInsertQuery = `INSERT INTO "FINANCEMANAGER"."Incomes" VALUES ('${currentUser.userID}', '${newIncomeID}', 
                                     ...
                                     ${newIncomeInfo.amount}, TO_DATE('${incomeDateInput}', 'YYYY-MM-DD HH24-MI-SS'))`;
        const financialIncomeUpdateQuery = `UPDATE "FINANCEMANAGER"."FinancialInfo"
                                            SET "Amount" = "Amount" + ${newIncomeInfo.amount},
                                            "Last Updated On" = TO_DATE('${incomeDateInput}', 'YYYY-MM-DD HH24-MI-SS')
