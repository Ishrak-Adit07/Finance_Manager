...
router.post("/", async(req, res)=>{
    ...
    currentUser.mail = req.body.username;
    ...
    if(check){
        ...
        currentUser.age = getAgeQueryResult[0][0];
        ...
        const collectAmountsInWalletsQuery = `SELECT "WalletID", "Amount"
                                              ...
                                              WHERE "UserID" LIKE '${currentUser.userID}'`;
        let collectAmountsInWalletsQueryResult = await runQuery(collectAmountsInWalletsQuery);
        for(var i=0; i<currentUser.wallets; i++){
            currentUser.amounts[collectAmountsInWalletsQueryResult[i][0]] = collectAmountsInWalletsQueryResult[i][1];
        }
